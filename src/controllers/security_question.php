...
$app->get('/security_questions/:user_id', function ($user_id) {
    try {
        $questions = security_question::with(relations::getFirstAccessUserRelations())->get();
...
$app->post('/security_question/save', function () use ($app) {
    try {
        ...
        if ($security_question->save()) {
            $data = security_question::with(relations::getFirstAccessUserRelations())->find($security_question->id);
            $error = new custonError(false, 0);
            return helpers::jsonResponse($error->parse_error(), $data);
        }
    } catch (Exception $ex) {
