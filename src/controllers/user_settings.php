...
$app->get('/user_settings/:id', function($id) {
    try {
        $data = user_settings::with('user.user_type','user.term','user.plan', 'settings')->find($id);
...
$app->get('/user_settings/user/:user', function($user_id) {
    ...
    try {
        $data = user_settings::with('user.user_type','user.term','user.plan', 'settings')
                ->where('user', '=', $user_id)
                ->get();
        $error = new custonError(false, 0);
        return helpers::jsonResponse($error->parse_error(), $data);
...
$app->post('/user_settings/save', function() use($app) {
    try {
        ...
        if ($user_settings->save()) {
            $data = user_settings::with('user.user_type','user.term','user.plan', 'settings')->find($user_settings->id);
            $error = new custonError(false, 0);
            return helpers::jsonResponse($error->parse_error(), $data);
        }
    } catch (Exception $ex) {
