...
$app->get('/user_privacy/:id', function($id) {
    try {
        $data = user_privacy::with(relations::getUserPrivacyRelations())->find($id);
...
$app->get('/user_privacy/user/:user', function($user_id) {
    ...
    try {
        $data = user_privacy::with(relations::getUserPrivacyRelations())
                ->where('user', '=', $user_id)
                ->get();
        $error = new custonError(false, 0);
        return helpers::jsonResponse($error->parse_error(), $data);
...
$app->post('/user_privacy/save', function() use($app) {
    try {
        ...
        if ($user_privacy->save()) {
            $data = user_privacy::with(relations::getUserPrivacyRelations())->find($user_privacy->id);
            $error = new custonError(false, 0);
            return helpers::jsonResponse($error->parse_error(), $data);
        }
    } catch (Exception $ex) {
