...
$app->post('/access_platform/:id/active', function ($id) use($app) {
    try {
        $access_platform = access_platform::find($id);
        $access_platform->is_active = $app->request()->post('is_active');
        $access_platform->user_last_update = $app->request()->post('user_admin');

        if ($access_platform->update()) {
            $data = access_platform::find($access_platform->id);
            $error = new custonError(false, 0);
            return helpers::jsonResponse($error->parse_error(), $data);
        }
